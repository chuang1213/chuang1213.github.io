...
/*********************** 上传项配置区 开始 ***********************/
//获取域名
$http = (_is_https() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'];
...
//支持上传的文件格式
$type = ['jpg','jpeg','png','gif','bmp','exe','flv','swf','mkv','avi','rm','rmvb','mpeg','mpg','ogg','ogv','mov','wmv','mp4','webm','mp3','wav','mid','rar','zip','tar','gz','7z','bz2','cab','iso','chm','doc','docx','xls','xlsx','ppt','pptx','pdf','txt','md','torrent'];
...
if(isset($_POST['iceEditor-img']) && $_POST['iceEditor-img']){
	$fileExt = _file_ext($_POST['iceEditor-img']);
	...
	if(!$img){
		...
		$name = _fileRename($info['basename']);
		//判断文件类型是否允许上传
...
foreach ($keys as $key){
	...
	//获取文件的名称
	$fileName = _file_pre($name);

	//获取文件的后缀名称
	$fileExt = _file_ext($name);

	//重命名
	$name = _fileRename($name,$key);
...
//获取文件后缀名，不包含 .
function _file_ext($name) {
	return strtolower(substr(strrchr($name, '.'), 1));
}

//获取文件的前缀，不包含 .
function _file_pre($name) {
	return substr($name, 0, strrpos($name, '.'));
}

//重命名
function _fileRename($name,$key=1) {
